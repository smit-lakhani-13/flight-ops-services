...
# download and push for a commit that is already in the registry.
#
...
# "not there" would push the image again and fail, because the repository's
# tags are immutable.
set -euo pipefail
...
if out=$(aws ecr describe-images \
    ...
    echo "$tag is already in ECR; reusing it, not pushing it again" >&2
    echo 'exists=true'
