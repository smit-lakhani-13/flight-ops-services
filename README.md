...
```bash
./mvnw clean verify                       # 261 tests: 252 run, 9 skipped, 0 failures
...
| Run | 277 | 0 failures (13 + 36 + 63 + 10 + 85 + 11 + 19 + 23 + 9 + 8) |
...
| Idempotency keys never expire. The key is a `NOT NULL` column of the booking row under `uk_bookings_idempotency_key`, so the unique index grows by one entry per booking | Keys valid for a stated window (Stripe removes them after 24 hours), after which a replay is a new request | `src/main/resources/db/migration/V4__booking_cancellation.sql` makes a cancelled booking keep its key, so a late replay returns the cancelled booking instead of booking again. A window would bound the index, but a replay after it would book again, a contract change clients have to be told about, and it needs a new migration, because applied ones are never edited. |
