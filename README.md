...
4. **The invariants live on the entity, not the service.** `Flight.reserveSeats` is the only way seats move in production, so no caller can forget the rules — including a caller written later by someone else. (`Flight.releaseSeats` also moves them and is tested, but nothing calls it: there is no cancel-booking endpoint yet. It is listed in the table below.)
