...
| What | State |
|---|---|
| The service | Built and tested: 266 tests, 9 of them on PostgreSQL 17 through Testcontainers, which run in CI and skip on a machine without Docker. Every endpoint exercised over HTTP against a running instance; `scripts/demo.sh` replays the tour. |
