...
- **Event transport decision.** [ADR 0015](adr/0015-event-transport.md)
  records why events go to an SQS standard queue and what a JMS broker
  would cost, from documentation.
...
- **Transport cost understated.** `ARCHITECTURE.md` said a new transport
  took one class; for a JMS broker it also takes a connection factory, a
  client library, a mode, a test and a new consumer.
