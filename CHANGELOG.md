...
release on, the strict rule applies: the next removal of a response field, or
...
- **`idempotencyKey` from `BookingDto`.** The list endpoint returns every
  ...
  only the response shape is. **This is the only field this release removes
  from a response.**
...
- **A losing replay got 409.** Two identical requests raced, and the loser
  caught the unique-constraint violation and reported a conflict. The correct
  answer is the winner's booking, with 201.
...
- **`findBookable` missed DELAYED flights.** A delayed flight is still bookable,
  but the query excluded it. A whole flight disappeared from the inventory, with
  no error, the moment it was delayed.
